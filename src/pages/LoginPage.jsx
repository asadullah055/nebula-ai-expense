...
const LoginPage = () => {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      const apiError =
        err.response?.data?.errors?.[0]?.msg || err.response?.data?.message || "Login failed";
      setError(apiError);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-violet-50 px-4 py-8 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <section className="mx-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-950 sm:p-7">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-100">Login</h1>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Access your account</p>
